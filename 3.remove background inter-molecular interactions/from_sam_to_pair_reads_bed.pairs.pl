...
open(OUTA,">intergene.rm_FalsePosi.bothUniq.bedpair") || die;
...
while(my $frag_a=<SM>){
	if($frag_a=~/^@/){
		...
		#print OUTA $chr_a,"\t",$loci_a-1,"\t",$loci_a+$match_a-1,"\t",$sub_a[0],"\t",255,"\t",$strand_a_new,"\t",$chr_b,"\t",$loci_b-1,"\t",$loci_b+$match_b-1,"\t",$sub_b[0],"\t",255,"\t",$strand_b_new,"\n";
		print OUTA $chr_a,"\t",$loci_a-1,"\t",$loci_a+$match_a-1,"\t",$chr_b,"\t",$loci_b-1,"\t",$loci_b+$match_b-1,"\t",$sample_a,"_",$id_a_info[0],"_",$id_a_info[1],"\t",255,"\t",$strand_a_new,"\t",$strand_b_new,"\n";
